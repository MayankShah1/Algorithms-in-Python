...
class LinkedBinaryTree(binary_tree.BinaryTree):
    ...
    class _Node:                    #Lightwweight , non-public class for storingg node
    ...
    def _delete(self, p):
        ...
        child = node._left if node._left else node._right       # might be None also
    ...
    def _attach(self, p, t1, t2):
        """
        Attach t1 and t2 as left and right subtrees of external p
        """
        node = self._validate(p)
        ...
        if not t1.is_empty():                                   # attach t1 as left subtree
            ...
            node._left = t1._root
            t1._root = None
            t1._size = 0                                        # set t1 instance as empty
        if not t2.is_empty():
            t2._root._parent = node
            node._right = t2._root
            t2._root = None
            t2._size = 0                                        # set t2 instance as empty
